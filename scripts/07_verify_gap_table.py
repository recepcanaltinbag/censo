...
def main() -> int:
    ...
    # COUNTED, not written out in words. The caption said "eighteen ... twenty
    # rows" through two rounds in which the surveyed set grew and this work
    # gained a module: by the time anyone read it the table had twenty-three.
    n_rows = len(results)
    n_assessed = sum(1 for k in results if "this work" not in k)
    ...
    B("\\caption{Concepts present in published water and observation "
      ...
      "fell below it. The rows marked \\emph{this work} are CENSO's own "
      "modules and are not among the assessed vocabularies, which is why the "
      f"text counts \\num{{{n_assessed}}} where the table has "
      f"\\num{{{n_rows}}} rows.}}")
    B("\\label{tab:gap}")
    ...
    # got most of it, and not all: the table still set 15 mm past the page,
    # because an l column cannot break a line and "limit of quantification, on
    # the analytical method" is one. \\resizebox is still refused -- it would
    # scale the rules and the text by different amounts and print at a size no
    # reader chose -- so the two text columns wrap instead, inside exactly the
    # width the page has.
    B("\\scriptsize\\setlength{\\tabcolsep}{3.5pt}")
    B("\\begin{tabularx}{\\textwidth}{@{}"
      ">{\\raggedright\\arraybackslash}X r "
      ">{\\raggedright\\arraybackslash}X " + "c" * len(order) + "@{}}")
    B("\\toprule")

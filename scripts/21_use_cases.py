...
CASES = [
    ...
    ("never_decidable",
     "A programme designer choosing what to monitor next",
     "Which substances has this network never once been able to decide, at "
     "any station, in any year?",
     "Reports them as compliant everywhere, so they look like the clean part "
     "of the programme and the budget goes elsewhere.",
     ...
     "Substances with method-insufficient assessments and no decided "
     "assessment of either kind: monitored, reported, and never once decided. "
     "A two-valued record cannot distinguish them from substances that were "
     "measured and found absent."),

    ("shape_check",
     "A data steward validating an incoming national submission",
     "Can I check this submission against a published constraint rather than "
     "a script I wrote myself?",
     "Offers no constraint to check against. Each recipient writes their own "
     "script, and what one accepts another rejects.",
     ...
     "The shapes ship with the vocabulary at a resolvable IRI, so the party "
     "receiving a submission and the party sending it can run the same check "
     "and disagree about the data rather than about the rules."),

    ("swap_effect",
     "A ministry asked what adopting a different standard would change",
     "If the threshold is swapped for another jurisdiction's, which "
     "assessments change and which are untouched?",
     "Has one threshold per substance, so the question requires editing the "
     "column and losing the previous answer. Nothing records which standard "
     "produced which verdict.",
     """SELECT ?package (COUNT(?t) AS ?thresholds) WHERE {
          ?t censo:definedBy ?package .
        } GROUP BY ?package""",
     "Every threshold cites the instrument that defines it, so both packages "
...
def main() -> int:
    ...
    WORDS = {7: "Seven", 8: "Eight", 9: "Nine", 10: "Ten",
             11: "Eleven", 12: "Twelve"}
    n_cases = WORDS.get(len(CASES), str(len(CASES)))
    A = [f"# {n_cases} questions the vocabulary exists to answer\n",

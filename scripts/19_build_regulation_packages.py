...
# Annex I entry 6 and footnote 9 of the consolidated Directive 2008/105/EC,
# read from refs/legal/EU-2008-105_consolidated-2026-05-10.pdf: the cadmium
# standards by water-hardness class. (class, minimum, maximum) in mg CaCO3/l --
# minimum inclusive, maximum exclusive, None open -- then the inland annual
# average and the maximum allowable concentration, ug/l. Class 1 is printed
# "<= 0,08" and "<= 0,45" and is emitted at those values.
#
# Until 2.4.0 the package emitted one cadmium threshold at the class-1 value
# with a single unsatisfiable condition, so the only verdict any cadmium
# observation could reach was PreconditionUnmet. The release reports hardness
# on rows of its own, so the class is decidable, and a package that states one
# value for five is stating the wrong law.
CD_CLASSES = [(1, None, 40, 0.08, 0.45), (2, 40, 50, 0.08, 0.45),
              (3, 50, 100, 0.09, 0.6), (4, 100, 200, 0.15, 0.9),
              (5, 200, None, 0.25, 1.5)]
# Annex I Part B point 3: "in the case of cadmium, lead, mercury and nickel ...
# the water EQS refer to the dissolved concentration".
DISSOLVED_METALS_CAS = {"7440-43-9", "7439-92-1", "7439-97-6", "7440-02-0"}
...
def emit_package(*, pkg_id, iri_base, title, description, jurisdiction,
                 ...
                 decision=None, fraction_cas=None, class_values=None):
    ...
    if decision:
        # The two choices that change verdicts and are policy rather than
        # vocabulary: how a censored result meets a standard, and the band a
        # measured value is read with when the record reports no uncertainty.
        L.append(f"    cereg:censoredResultRule "
                 f"cereg:{decision['censored_rule']} ;")
        L.append(f"    cereg:defaultUncertaintyFactor "
                 f"{lit(decision['u_factor'])} ;")
    if transposes:
    ...
    for r in rows:
        ...
        cas_row = [c.strip() for c in
                   str(r.get("all_cas") or r.get("cas") or "").split(";")
                   if c.strip()]
        classes = (next((class_values[c] for c in cas_row
                         if c in (class_values or {})), None)
                   if cond and cond[0] == "hardness-class" else None)
        dissolved = (not is_group) and any(c in (fraction_cas or ())
                                           for c in cas_row)
        if dissolved:
            fraction_used = True
        for kind, value, cls in (("aa", r.get("aa"),
            ...
            if v is None:
                continue
            base_conds = ["dissolved"] if dissolved else []
            if classes:
                for k, lo, hi, aa_v, mac_v in classes:
                    thresholds.append((
                        f"cereg:{pkg_id}-{s}-{kind}-class-{k}",
                        kind_cls or cls, aa_v if kind == "aa" else mac_v,
                        subject_iri, r.get("note", ""),
                        f"{r['name']}, hardness class {k}", kind, is_group,
                        base_conds + [f"hardness-class-{k}"]))
                    class_used.add((k, lo, hi))
                    n_aa += kind == "aa"
                    n_mac += kind == "mac"
                continue
            ...
            thresholds.append((t_iri, kind_cls or cls, v, subject_iri,
                               ...
                               base_conds + ([cond[0]] if cond else [])))
            if is_group:
                n_grp += 1
            elif kind == "aa":
                n_aa += 1
    ...
    if class_used:
        # the per-class conditions replace the single one footnote 9 names
        conditions_used.pop("hardness-class", None)
    ...
    if class_used or fraction_used:
        L.append("#" * 78)
        L.append("#  Conditions a record can meet: the fraction measured, and the")
        L.append("#  hardness class of the water, one condition per class")
        L.append("#" * 78)
        L.append("")
    if fraction_used:
        L.append(f"cereg:{pkg_id}-cond-dissolved a censo:FractionCondition ;")
        L.append(f'    rdfs:label "dissolved fraction ({pkg_id})"@en ;')
        L.append('    rdfs:comment "Annex I Part B point 3: for cadmium, lead, '
                 'mercury and nickel the water EQS refer to the dissolved '
                 'concentration, the dissolved phase of a water sample obtained '
                 'by filtration through a 0,45 um filter or any equivalent '
                 'pre-treatment."@en .')
        L.append("")
    for k, lo, hi in sorted(class_used):
        span = (f"< {hi}" if lo is None else f">= {lo}" if hi is None
                else f"{lo} to < {hi}")
        L.append(f"cereg:{pkg_id}-cond-hardness-class-{k} a "
                 f"censo:HardnessClassCondition ;")
        L.append(f'    rdfs:label "hardness class {k}, {span} mg CaCO3/l '
                 f'({pkg_id})"@en ;')
        L.append("    censo:requiresCovariate cereg:covariate-hardness ;")
        if lo is not None:
            L.append(f"    censo:covariateMinimum {lit(lo)} ;")
        if hi is not None:
            L.append(f"    censo:covariateMaximum {lit(hi)} ;")
        L.append("    censo:covariateUnit unit:MilliGM-PER-L ;")
        L.append(f'    rdfs:comment "Annex I footnote 9, class {k}: {span} mg '
                 f'CaCO3/l. Hardness is expressed as calcium carbonate."@en .')
        L.append("")
    if class_used and "hardness" not in {
            c for _, (_, _, cs, _) in conditions_used.items() for c in cs}:
        L.append("cereg:covariate-hardness a sosa:ObservableProperty ;")
        L.append('    rdfs:label "hardness"@en .')
        L.append("")
...
def main() -> int:
    ...
    if p.exists():
        ...
        out, n_a, n_aa, n_mac = emit_package(
            ...
            # 2.2.0: cadmium is five thresholds by hardness class where it was
            # one, the four metals carry the dissolved-fraction condition, and
            # the package states its decision rule. A consumer of 2.1.0 that
            # looked up cadmium by one IRI would break.
            version="2.2.0", rows=rows,
            analyte_prefix="cereg:analyte-eu-",
            decision={"censored_rule": "PointComparison", "u_factor": 0.5},
            fraction_cas=DISSOLVED_METALS_CAS,
            class_values={"7440-43-9": CD_CLASSES})

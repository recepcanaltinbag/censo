...
def verdict(thr, value_ug, loq_ug, below_loq, *, status=None, cas="",
            condition=None, fraction="", hardness=None, dissolved_cas=None,
            censored_rule="point", u_factor=_m.LEGAL_UNCERTAINTY_AT_EQS):
    ...
    return _MAP[_m.assess(status, value_ug, loq_ug, thr, cas=cas,
                          ...
                          hardness=hardness, dissolved_cas=dissolved_cas,
                          censored_rule=censored_rule, u_factor=u_factor)[0]]
...
def main() -> int:
    ...
    # EVERY DIFFERENCE BETWEEN THE COLUMNS NOW COMES FROM A FILE. The conditions
    # a package attaches, the fraction its metal standards are written on, how it
    # compares a censored result and what band it reads a measurement with are
    # all read from the package itself. They used to be a Python branch on the
    # jurisdiction's name -- `eu = name == "EU"` -- which made "swap the package
    # and the verdicts change" true of the thresholds and false of the rule.
    DEC = {name: _m.package_decision(path) for name, path, _ in PACKAGES}
    for name, d in DEC.items():
        print(f"  {name}: censored-result rule {d['censored_rule']}, "
              f"U = {d['u_factor']:g} T, "
              f"{len(d['conditions'])} conditional analyte(s), "
              f"{len(d['fraction_cas'])} on the dissolved fraction"
              + ("" if d["conditions"] or d["fraction_cas"]
                 else "  (states none; the vocabulary's default applies)"))
    ...
    for row in rows:
        ...
        for name, thr, _ in packs:
            ...
            v = verdict(thr.get(cas), val_ug, loq_ug, status == "censored",
                        ...
                        condition=d["conditions"].get(cas),
                        dissolved_cas=d["fraction_cas"],
                        censored_rule=d["censored_rule"],
                        u_factor=d["u_factor"], **applic)
            tally[name][v] += 1
            vs.append(v)
        cross[tuple(vs)] += 1
    ...
    L = ["# The same observations under two regulations", "",
         ...
         "columns; only the regulation package differs -- including the rule "
         "it states for a censored result, the band it reads a measurement "
         "with, and the conditions it attaches to its thresholds, all read "
         "from the package file.", "",]
    L += [" - " + f"**{name}**: censored-result rule `{DEC[name]['censored_rule']}`, "
          f"U = {DEC[name]['u_factor']:g} T, "
          f"{len(DEC[name]['conditions'])} conditional analyte(s)"
          + ("" if DEC[name]["conditions"] or DEC[name]["fraction_cas"]
             else " — the package states no condition and no rule of its own, "
                  "so the vocabulary's default is applied and said so here")
